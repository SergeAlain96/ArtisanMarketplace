...
export default async function ArtisanDetailPage({ params }) {
  ...
  if (!details) {
    return <div className="card">Artisan introuvable.</div>;
  }
  ...
  return (
    <section className="space-y-6">
      <div className="card border-0 bg-gradient-to-br from-white to-brand-50/50 dark:from-slate-900 dark:to-slate-800">
        ...
        <p className="mt-3 max-w-3xl text-slate-700 dark:text-slate-300">{details.profile?.bio || 'Pas de biographie.'}</p>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{details.profile?.location || 'Localisation non définie'}</p>
        <div className="mt-4 inline-flex items-center gap-3 rounded-xl bg-white px-3 py-2 shadow-sm dark:bg-slate-900 dark:shadow-none">
          ...
          <span className="text-sm text-slate-600 dark:text-slate-300">{ratings.average} / 5 ({ratings.count} avis)</span>
        </div>
      </div>

      <div>
        <h2 className="mb-3 text-xl font-semibold tracking-tight">Produits</h2>
